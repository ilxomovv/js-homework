...
// Load posts from PHP backend
async function loadPosts() {
    const postsContainer = document.getElementById('postsContainer');
    if (!postsContainer) return;
    
    postsContainer.innerHTML = '<div class="loading">Postlar yuklanmoqda...</div>';
    ...
    try {
        ...
        if (data.success) {
            ...
            postsContainer.innerHTML = '<div class="error">Xatolik: ' + data.message + '</div>';
            // Load demo posts if PHP doesn't work
            loadDemoPosts();
        }
    } catch (error) {
        console.log('PHP backend not available, using demo data');
...
function likePost(postId) {
    ...
    if (post) {
        ...
        displayPosts(currentCategory === 'all' ? posts : posts.filter(p => p.category === currentCategory));
        alert('Like +1! 👍');
    }
}
...
function showComments(postId) {
    const post = posts.find(p => p.id === postId);
    if (post) {
        alert(`Izohlar soni: ${post.comments || 0}\n\nBu demo versiya - real PHP backend bilan ishlaydi!`);
    }
}
